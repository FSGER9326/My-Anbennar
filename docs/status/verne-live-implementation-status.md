...
| Mission rewrite / mission integration | **Partial** | `missions/Verne_Missions.txt`, `common/scripted_triggers/verne_overhaul_triggers.txt`, `common/scripted_effects/verne_overhaul_effects.txt`, `localisation/verne_overhaul_l_english.yml` | Slot headers (slots 5–9) and Liliac-lane prerequisite wiring (7 missions) corrected 2026-04-02. The mission tree carries helper-layer integrations, variables, flags, and state hooks, but a full audit against the doctrine/reform/dynasty/order/crisis layer is still pending. |
