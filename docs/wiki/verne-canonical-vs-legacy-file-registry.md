...
| Overhaul on_actions (scaffold shell) | `common/on_actions/zz_verne_overhaul_on_actions.txt` | Transitional | Keep hook shell + shared wiring only; avoid duplicating full dynasty logic here. | Exists as merge-stable scaffold anchor; overlaps with dynasty-specific override file. |
| Dynasty subsystem on_actions | `common/on_actions/zzz_verne_overhaul_dynasty_on_actions.txt` | Canonical (dynasty subsystem) | Edit here for active dynasty safeguard dispatch behavior. | Currently the live `on_new_heir` event dispatch implementation. |
| Overhaul dynasty events (scaffold shell) | `events/verne_overhaul_dynasty_events.txt` | Transitional | Keep placeholder/no-op baseline unless intentionally promoting it to live implementation. | Scaffold file conflicts by ID with `zzz_*` dynasty event file; do not implement both in parallel. |
| Dynasty subsystem events | `events/zzz_verne_overhaul_dynasty_events.txt` | Canonical (dynasty subsystem) | Edit here for live heir safeguard event logic. | Contains active trigger/effect chain for fallback heir generation. |
| Overhaul trigger split | `common/scripted_triggers/zzz_verne_overhaul_dynasty_triggers.txt` | Canonical (dynasty subsystem) | Edit here for dynasty-only trigger logic. | Isolated trigger split reduces hotspot churn in wider trigger library. |
...
| Dynasty localization split | `localisation/zzz_verne_overhaul_dynasty_l_english.yml` | Canonical (dynasty subsystem) | Keep dynasty event loc here while `zzz` dynasty event file remains canonical. | Intentionally overrides scaffold-era dynastic localization keys with live text. |
...
2. **No new placeholder keys:** new production text belongs in `localisation/verne_overhaul_l_english.yml` or the active dynasty loc split, not scaffold localization.
